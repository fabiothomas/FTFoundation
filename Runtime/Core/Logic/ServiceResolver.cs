...
namespace FTFoundation.Core
{
  ...
  internal static class ServiceResolver
  {
    ...
    // Keyed by Scene.handle, which is unique per loaded instance of a scene
    ...
    internal static void Clear()
    {
      ...
      // Both are [ThreadStatic] and, on the happy path, already reset to null/empty by the
      // try/finally blocks around whatever set them.
      CurrentTransientContext = null;
    ...
    internal static object? GetService(Type _interface, int sceneHandle, ServiceTargetData target, bool optional)
    {
      ...
      switch (type)
      {
        ...
        case ServiceType.SCOPED:
          ...
          object newScoped;
          List<object> ownedTransients;
          try
          {
            newScoped = CreateService(service, _interface, sceneHandle, ServiceTargetDataType.SCOPED, ServiceTargetData.EmptyServiceTargetData());
          }
          finally
          {
            ...
            ownedTransients = CurrentTransientContext;
    ...
    private static object GetMultiService(Type elementType, int sceneHandle, ServiceTargetData target)
    {
      ...
      if (!ServiceProvider.multiServiceCache.TryGetValue(elementType, out var serviceTypes)) return list;
      ...
      foreach (var concreteType in serviceTypes)
      {
        ...
        try
        {
          switch (attr.Type)
          {
            ...
            case ServiceType.SINGLETON:
              ...
              if (ServiceProvider.serviceCache.TryGetValue(elementType, out var winner)
                  && winner == concreteType
        ...
        {
          // One failing implementation must not take down every other active implementation
          // of this interface. For example one broken ILoggerSink should not silence all logging.
          // Goes straight to Debug.LogError (not a DI-resolved logger) since that logger may
          // itself depend on the interface currently being resolved.
          Debug.LogError($"[FTFoundation] Failed to construct '{concreteType.Name}' for multi-service '{elementType.Name}': {e.Message}");
          continue;
        }
