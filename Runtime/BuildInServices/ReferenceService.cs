...
namespace FTFoundation.BuildInServices
{
  ...
  public class ReferenceService : IReferenceService
  {
    ...
    public IDisposable RegisterReference(MonoBehaviour ref_)
    {
      if (!ref_)
      {
        LoggerService.LogWarning("Could not register object because no reference was given");
      ...
      {
        LoggerService.LogError($"{type.Name} is already assigned and can not be assigned multiple times. Please ensure there is only one {type.Name} active at a time");
        return new DelegateDisposable(() => { });
      }
    ...
    {
      // Only remove if this disposable's instance is still the one on file
      // A later registration for the same type may have already replaced it.
    ...
    public T? GetReference<T>() where T : MonoBehaviour
    {
      ...
      LoggerService.LogWarning($"Object '{type.Name}' could not be found. Make sure to add this object to the scene exactly once");
      return default;
    }
