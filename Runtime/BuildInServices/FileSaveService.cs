...
namespace FTFoundation.BuildInServices
{
  ...
  public class FileSaveService : IFileSaveService, IServiceCleanup
  {
    ...
    private void OnUpdate()
    {
      ...
      if (_timeSinceLastFlush >= FlushInterval) Flush();
    }
    ...
    private static void DeserializeInto(string text, Dictionary<string, string> target)
    {
      foreach (string line in text.Split('\n'))
      {
        ...
        if (sep < 0) continue;
        string key = Encoding.UTF8.GetString(Convert.FromBase64String(line[..sep]));
        string value = Encoding.UTF8.GetString(Convert.FromBase64String(line[(sep + 1)..]));
        target[key] = value;
      }
    }
